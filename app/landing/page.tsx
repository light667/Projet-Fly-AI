...
import { Sparkles, Brain, Target, FileText, TrendingUp, Globe, Smartphone, Zap, Shield, ArrowRight, Mail, Linkedin, Check } from "lucide-react";
    ...
    <header className="fixed top-0 inset-x-0 z-50 px-4 pt-4">
      <nav className="max-w-6xl mx-auto glass rounded-2xl px-5 py-3 flex items-center justify-between">
        <a href="#top" className="flex items-center gap-2.5">
          <div className="h-9 w-9 rounded-xl shadow-glow"><Logo className="w-9 h-9" /></div>
        ...
        <a href="#contact" className="gradient-primary text-primary-foreground text-sm font-medium px-4 py-2 rounded-xl hover:opacity-90 transition shadow-glow">
          Nous contacter
        </a>
      </nav>
    </header>
    ...
    <section id="top" className="relative pt-40 pb-24 px-4 bg-hero-glow">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 animate-fade-up">
          L'intelligence artificielle qui aide les étudiants africains à trouver des bourses,
        ...
        <div className="relative mx-auto max-w-sm animate-float">
          ...
          <div className="relative w-48 h-48 mx-auto rounded-[2rem] animate-pulse-glow"><Logo className="w-48 h-48" /></div>
        </div>
      </div>
    </section>
  );
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={onSubmit} className="glass rounded-3xl p-6 md:p-8 space-y-4 shadow-card">
          <div className="grid md:grid-cols-2 gap-4">
            <input required name="name" placeholder="Votre nom" maxLength={100} className="bg-input border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition" />
            <input required type="email" name="email" placeholder="Votre email" maxLength={255} className="bg-input border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition" />
          </div>
          <input name="subject" placeholder="Sujet" maxLength={150} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition" />
          <textarea required name="message" placeholder="Votre message..." rows={5} maxLength={2000} className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition resize-none" />
          <button type="submit" disabled={status === 'loading'} className="w-full gradient-primary text-primary-foreground font-medium px-6 py-3.5 rounded-xl hover:opacity-90 transition shadow-glow inline-flex items-center justify-center gap-2 disabled:opacity-60">
            {status === 'loading' ? 'Envoi...' : status === 'sent' ? 'Message envoyé ✓' : 'Envoyer le message'}
            {status === 'idle' && <ArrowRight className="h-4 w-4" />}
          </button>
          {status === 'error' && <p className="text-sm text-destructive text-center">Erreur lors de l'envoi. Réessayez ou écrivez-nous directement.</p>}
        </form>

        <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
          <a href="mailto:nethaniahdjossou@gmail.com" className="glass rounded-xl px-5 py-3 inline-flex items-center gap-2 text-sm hover:bg-muted/40 transition"><Mail className="h-4 w-4 text-secondary" /> nethaniahdjossou@gmail.com</a>
          <a href="https://www.linkedin.com/company/flyai-org/" target="_blank" rel="noreferrer" className="glass rounded-xl px-5 py-3 inline-flex items-center gap-2 text-sm hover:bg-muted/40 transition"><Linkedin className="h-4 w-4 text-secondary" /> LinkedIn Fly AI</a>
        </div>
      </div>
    </section>
  );
    ...
    <footer className="px-4 py-12 border-t border-border">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg"><Logo className="w-8 h-8" /></div>
          <div>
        ...
        <div className="text-xs text-muted-foreground">© {new Date().getFullYear()} Fly AI. Tous droits réservés.</div>
        <div className="flex items-center gap-3">
          <a href="mailto:nethaniahdjossou@gmail.com" className="w-9 h-9 rounded-lg glass flex items-center justify-center hover:bg-muted/40 transition"><Mail className="h-4 w-4" /></a>
          <a href="https://www.linkedin.com/company/flyai-org/" target="_blank" rel="noreferrer" className="w-9 h-9 rounded-lg glass flex items-center justify-center hover:bg-muted/40 transition"><Linkedin className="h-4 w-4" /></a>
        </div>
      </div>
    </footer>
  );
